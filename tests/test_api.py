...
def _student(sid: str, with_indicators: bool = True) -> Student:
    return Student(
        student_id=sid,
        name=f"Student {sid}",
        program="B.Sc. Computer Science",
        enrollment_date=date(2025, 10, 1),
        indicators=_indicators() if with_indicators else None,
    )
...
def client() -> TestClient:
    repo = FakeRepo([_student("S0001"), _student("S0002", with_indicators=False)])
